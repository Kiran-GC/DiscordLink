    ...
    wrapGenerate: async ({ doGenerate, params, model }) => {
      return withSpanHandling(
        ...
        async (span, commonContext, _lease) => {
          ...
          appendPromptMetadataToSpan(span, params.prompt);
          setScopeAttributes(span);
          setPreCallAttributesV3(span, params, context3, model);
          const res = await doGenerate();
          await setPostCallAttributesV3(span, res, context3, model);
          return res;
        },
        { version: "v3" }
      );
                  ...
                  try {
                    ...
                    await setPostCallAttributesV3(
                      span,
                      {
                        ...
                        content: toolAggregator.result.length > 0 ? toolAggregator.result : textAggregator.text ? [{ type: "text", text: textAggregator.text }] : [],
                        warnings: []
                      },
                      context3,
                      model
...
function setPreCallAttributesV3(span, options, context3, model) {
  const redactionPolicy = getRedactionPolicy();
  setBaseAttributes(span, model.provider, model.modelId);
  ...
  context3.originalV3Prompt = options.prompt;
  context3.originalPrompt = processedPrompt;
  handleMaybeRedactedAttribute(
    span,
    Attr.GenAI.Input.Messages,
...
async function setPostCallAttributesV3(span, result, context3, _model) {
  const redactionPolicy = getRedactionPolicy();
  const toolCalls = result.content?.filter(
    (c) => c.type === "tool-call"
  );
  ...
  if (toolCalls && toolCalls.length > 0) {
    ...
    const toolResultsMap = extractToolResultsFromPromptV3(context3.originalV3Prompt || []);
    const textContent = result.content?.find((c) => c.type === "text");
    const assistantText = textContent?.type === "text" ? textContent.text : void 0;
    const updatedPrompt = appendToolCalls(
      originalPrompt,
...
async function processToolCallsAndCreateSpansV3(parentSpan, content) {
  const redactionPolicy = getRedactionPolicy();
  const textContent = content.find((c) => c.type === "text");
  const assistantText = textContent?.type === "text" ? textContent.text : void 0;
  const toolCalls = content.filter((c) => c.type === "tool-call");
  if (toolCalls.length === 0) {
    const sanitizableContent = content.filter(
      (c) => c.type === "text" || c.type === "reasoning" || c.type === "file" || c.type === "source" || c.type === "tool-result"
    );
    const completion = [
      {
        role: "assistant",
...
// src/otel/AxiomWrappedLanguageModelV3.ts
function isLanguageModelV3(model) {
  return model != null && typeof model === "object" && "specificationVersion" in model && "provider" in model && "modelId" in model && model.specificationVersion === "v3" && typeof model.provider === "string" && typeof model.modelId === "string";
}
var AxiomWrappedLanguageModelV3 = class {
  constructor(model) {
...
function wrapAISDKModel(model) {
  if (isLanguageModelV3(model)) {
    return new AxiomWrappedLanguageModelV3(model);
...
function assertZodV4(schema, context3) {
  if (!isZodV4Schema(schema)) {
    throw new Error(
      `[AxiomAI] Zod v4 schemas are required (detected in ${context3}). Found unsupported Zod version.`
    );
  }
}
