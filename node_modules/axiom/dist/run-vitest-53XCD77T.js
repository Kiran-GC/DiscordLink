...
var evalsRunnerPath = resolve(getCurrentDir(), "evals", "custom-runner.js");
var resolveVitestConfigPath = (vitestConfig, configPath, rootDir) => {
...
var runVitest = async (dir, opts) => {
  ...
  const evalTimeoutMs = opts.config?.eval?.timeoutMs || 6e4;
  const hookTimeoutMs = Math.max(evalTimeoutMs, 6e4);
  ...
  const vi = await createVitest(
    "test",
    {
      ...
      // Vitest default is 10s, which might not be enough to flush OTLP exporters and update the evaluation via API in `afterAll`
